...
from wycode.serialization import (
...
class AnthropicClient(LLMClient):
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .wycode/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    async def stream(
        ...
        _mark_last_user_tail_for_cache(messages) # 标记最后一条 user 消息的尾部
        ...
        try:
            async with self._client.messages.stream(**kwargs) as stream:
                ...
                final = await stream.get_final_message() # 获取最终消息
                usage = final.usage
    ...
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .wycode/config.yaml or via OPENAI_API_KEY env var."
            )
