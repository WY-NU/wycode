...
from wycode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from wycode.commands.completion import CompletionPopup
from wycode.commands.handlers import register_all_commands
from wycode.config import MCPServerConfig, ProviderConfig
from wycode.hooks import HookContext, HookEngine, load_hooks
from wycode.conversation import ConversationManager, Message
from wycode.mcp import ConnectResult, MCPManager
from wycode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from wycode.permissions import (
...
from wycode.agents.loader import AgentLoader
from wycode.agents.task_manager import TaskManager
from wycode.agents.trace import TraceManager
from wycode.agents.notification import inject_task_notifications
from wycode.commands.handlers.tasks import create_tasks_command
from wycode.skills.executor import SkillExecutor
from wycode.skills.loader import SkillLoader
from wycode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from wycode.cache import FileCache
from wycode.tools import ToolRegistry, create_default_registry
from wycode.tools.agent_tool import AgentTool
from wycode.tools.ask_user import AskUserEvent, AskUserTool
from wycode.tools.impl.tool_search import ToolSearchTool
from wycode.tools.install_skill import InstallSkillTool
from wycode.tools.load_skill import LoadSkill
from wycode.worktree.cleanup import start_stale_cleanup_task
from wycode.worktree.manager import WorktreeManager
from wycode.commands.handlers.worktree import create_worktree_command
from wycode.teammate_tree import TeammateTree
...
class ChatInput(TextArea):
    BINDINGS = [
        ...
        Binding("ctrl+up", "nav_up", "Previous history", priority=True),
        Binding("ctrl+down", "nav_down", "Next history", priority=True),
    ]
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".wycode" / "history"
...
class WyCodeApp(App):
    ...
    def __init__(
        ...
        from wycode.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_WYCODE_THEME)
        self.theme = "wycode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".wycode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".wycode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".wycode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from wycode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.wycode/config.yaml",
                        f"{work_dir}/.wycode/permissions.local.yaml",
                    ],
        ...
        from wycode.filehistory import FileHistory
        ...
        from wycode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from wycode.config import WorktreeConfig
        ...
        from wycode.tools.enter_worktree import EnterWorktreeTool
        from wycode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from wycode.teams.manager import TeamManager
        from wycode.tools.team_create import TeamCreateTool
        from wycode.tools.team_delete import TeamDeleteTool
        ...
        from wycode.commands.handlers.trace import create_trace_command
        ...
        from wycode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from wycode.tools.base import StreamEnd, TextDelta
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        if self._mcp_init_task and not self._mcp_init_task.done():
            self._show_system_message(
                "MCP servers are still connecting; continuing without MCP tools for this message."
            )
    ...
    async def _show_plan_approval(self) -> None:
        from wycode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from wycode.plan_dialog import InlinePlanWidget, PlanChoice
        from wycode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from wycode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from wycode.permission_dialog import InlinePermissionWidget
