...
class ProviderConfig:
    name: str # 供应商名称
    protocol: str # 供应商协议
    base_url: str # 供应商 API 根 URL
    model: str # 模型名称
    api_key: str = "" # API 密钥
    thinking: bool = False # 是否启用「思考中」模式
    # 0 表示"未设置" — get_context_window() 通过四层 fallback 解析真实窗口大小。
    # 正数表示配置文件里显式指定的覆盖值。
    context_window: int = 0 # 模型 context window
    max_output_tokens: int = 0 # 模型 max output tokens
    # 运行时 cache，存放从 provider 的 /v1/models 端点自动拉取的 context window
    # （get_context_window 的第 2 层）。通过 set_fetched_context_window() 写入一次；
    # 0 表示"尚未拉取"。不会持久化。
    _fetched_context_window: int = field(default=0, repr=False) # cache
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".wycode" / "config.yaml",
        cwd / ".wycode" / "config.yaml",
        cwd / ".wycode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .wycode/config.yaml "
            "in project or ~/.wycode/config.yaml"
        )
    return merged
