...
from wycode.client import LLMClient
from wycode.context import (
    CompactBoundary,
...
from wycode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from wycode.conversation import ThinkingBlock as ConvThinkingBlock
from wycode.memory.auto_memory import MemoryManager
from wycode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from wycode.hooks import HookContext, HookEngine, ToolRejectedError
from wycode.hooks.engine import HookNotification
from wycode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from wycode.tools import ToolRegistry
from wycode.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
- text: LLM 返回的文本内容
- tool_calls: 工具调用列表
- thinking_blocks: LLM 思考块列表
- stop_reason: LLM 停止的原因
- input_tokens: 输入的 token 数量
- output_tokens: 输出的 token 数量
- cache_read: 缓存读取的 token 数量
- cache_creation: 缓存创建的 token 数量
"""
@dataclass
class LLMResponse:
    text: str = "" # LLM 输出的文本内容
    tool_calls: list[ToolCallComplete] = field(default_factory=list)# 工具调用列表
    thinking_blocks: list[ThinkingBlock] = field(default_factory=list)# LLM 思考块列表
    stop_reason: str = ""# LLM 停止的原因
    input_tokens: int = 0# 输入的 token 数量
    output_tokens: int = 0# 输出的 token 数量
    cache_read: int = 0# 缓存读取的 token 数量
    cache_creation: int = 0# 缓存创建的 token 数量
...
class Agent:
    def __init__(
        ...
        if memory_manager is not None:
            from wycode.memory.consolidation import MemoryConsolidator
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".wycode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".wycode" / "plans"
    ...
    async def _run_post_tool_hook(self, tc: ToolCallComplete) -> None:
        if not self.hook_engine:
            return
    ...
    async def run(self, conversation: ConversationManager) -> AsyncIterator[AgentEvent]:
        ...
        conversation.inject_environment(env_context) # 注入环境信息

        memory_content = self.memory_manager.load() if self.memory_manager else ""
        conversation.inject_long_term_memory(self.instructions_content, memory_content) # 注入长期记忆和指令内容
        ...
        Agent 主循环：
        1. 检查 max_iterations
        2. 执行 turn_start hook
        3. 消费 mailbox
        4. 执行 pre_send hook
        5. 构建 system prompt
        6. 如果 plan_mode，添加 plan reminder
        7. 执行 auto_compact
        8. 调用 LLMClient.stream 获取流式响应
        9. 收集流式响应，处理 tool_calls
        10. 如果没有 tool_calls，添加 assistant 消息，提取记忆，执行 consolidator，执行 turn_end 和 session_end hook，完成循环
        11. 如果有 tool_calls，添加 assistant 消息，记录使用量锚点，执行工具，收集工具结果，处理工具结果事件
        12. 如果连续 unknown 工具结果超过阈值，终止循环
        13. 如果 stop_reason 是 max_tokens，尝试恢复输出
        14. 如果没有 tool_calls，完成循环
        15. 如果有 tool_calls，继续下一轮循环
        """
        ...
        while True:
            ...
            if self.plan_mode:
                ...
                conversation.add_system_reminder(plan_reminder) # 添加计划提示
            ...
            tools = self.registry.get_all_schemas(self.protocol) # 获取所有工具
            ...
            """
            调用 LLMClient.stream 获取流式响应，使用 StreamCollector 收集响应事件。
            """
            ...
            conversation.add_tool_results_message(tool_results) # 将工具结果加入对话历史
    ...
    async def _execute_single_tool_direct(
        ...
        hook_result = await self._run_pre_tool_hook(tc)
        ...
        await self._run_post_tool_hook(tc)
    ...
    async def _execute_tool(
        ...
        if hook_result is not None:
            elapsed = time.monotonic() - start
            yield hook_result, elapsed, is_unknown
            return
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from wycode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from wycode.context.manager import (
