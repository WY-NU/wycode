...
from wycode.worktree.models import WorktreeSession
...
def _session_path(wycode_dir: Path) -> Path:
    return wycode_dir / SESSION_FILENAME
...
def load_worktree_session(wycode_dir: Path) -> WorktreeSession | None:
    path = _session_path(wycode_dir)
    if not path.exists():
        return None
    try:
