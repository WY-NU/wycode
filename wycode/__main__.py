...
from wycode.config import ConfigError, load_config
from wycode.hooks import HookConfigError, HookEngine, load_hooks
from wycode.permissions import PermissionMode


def main() -> None:
    # 先确保 .wycode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".wycode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".wycode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="wycode", description="WyCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from wycode.remote import RemoteServer
    ...
    from wycode.app import WyCodeApp
    ...
    if sys.platform != "win32" or os.environ.get("WYCODE_NO_ALT_SCREEN") == "1":
        from wycode.driver import NoAltScreenDriver
        app_kwargs["driver_class"] = NoAltScreenDriver

    app = WyCodeApp(**app_kwargs)
    app.run()
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from wycode.client import create_client, resolve_context_window
    from wycode.conversation import ConversationManager
    from wycode.memory.instructions import load_instructions
    ...
    from wycode.tools import create_default_registry
